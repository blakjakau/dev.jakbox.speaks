...
func (api *API) handleStream(w http.ResponseWriter, r *http.Request) {
	...
	go func() {
		defer close(done)
		var infRepl *strings.Replacer = strings.NewReplacer()
		var dispRepl *strings.Replacer = strings.NewReplacer()
		...
		for {
			...
			case req, ok := <-textInput:
				if !ok {
					...
					api.flushRemaining(conn, &textBuffer, state, state.ls, state.ns, state.nw, state.varp, infRepl, dispRepl)
					return
				}

				state.isAnnotated = req.annotated
				state.ls = req.ls
				state.ns = req.ns
				state.nw = req.nw
				state.varp = req.varp
				...
				var marked string
				textBuffer += req.text
				marked, infRepl, dispRepl = ApplyPreSplit(textBuffer)
				textBuffer = marked
				...
				for {
					...
					api.synthesizeAndSend(conn, segment, state, state.ls, state.ns, state.nw, state.varp, infRepl, dispRepl)
				}
...
func (api *API) synthesizeAndSend(conn *websocket.Conn, text string, state *sessionState, baseLS, baseNS, baseNW, variance float32, infRepl, dispRepl *strings.Replacer) {
	text = strings.TrimSpace(text)
	if text == "" {
		return
	}
	
	displayText := strings.TrimSpace(dispRepl.Replace(text))
	text = infRepl.Replace(text)
	text = ApplyPostSplit(text)
	if text == "" {
		return
	}

	log.Printf("[WS] Engine Synthesis Start: '%s' (Display: '%s')", text, displayText)
...
func (api *API) flushRemaining(conn *websocket.Conn, buffer *string, state *sessionState, ls, ns, nw, variance float32, infRepl, dispRepl *strings.Replacer) {
	text := *buffer
	if text == "" {
		return
	}
...
Loop:
	for {
		...
		if splitIdx == -1 {
			...
			api.synthesizeAndSend(conn, text, state, ls, ns, nw, variance, infRepl, dispRepl)
			break Loop
		}
		...
		api.synthesizeAndSend(conn, segment, state, ls, ns, nw, variance, infRepl, dispRepl)
	}
